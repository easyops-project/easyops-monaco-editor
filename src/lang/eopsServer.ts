...
export function start(reader: MessageReader, writer: MessageWriter): EOPSServer {
...
export class EOPSServer {
    ...
    constructor(protected readonly connection: IConnection) {
        ...
        this.connection.onCompletion((_textDocumentPosition: TextDocumentPositionParams) => this.onCompletion(_textDocumentPosition));
    ...
    getEndingIndex(startingIndex: number, charCount: number, error: string, lines: string[], lineNo: number) {
        switch (error) {
    ...
    async validateTextDocument(textDocument: TextDocument): Promise<void> {
        ...
        for (const validationError of validationErrors) {
            ...
            let startingIndex: number = (this.getStartingIndex(lines, validationError.lineNumber) + validationError.characterIndex);
            let diagnostic: Diagnostic = {
                severity,
                range: {
                    start: textDocument.positionAt(startingIndex),
                    end: textDocument.positionAt(this.getEndingIndex(startingIndex, validationError.characterCount, validationError.errorMessage, lines, validationError.lineNumber))
                },
        ...
        return [
            {
                ...
                data: 1,
                insertText: "using aws"
            },
            {
                label: 'Use Azure',
                kind: CompletionItemKind.Snippet,
                data: 2,
                insertText: "using azure"
            },
            {
                label: 'Create VM',
                kind: CompletionItemKind.Snippet,
